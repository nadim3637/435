...
const getAvailableKeys = async (): Promise<string[]> => {
    ...
    // 1. Try Global Secure Keys from RTDB (Direct Read)
    try {
        const snap = await get(ref(rtdb, 'system_settings/groqApiKeys'));
        if (snap.exists()) {
            const data = snap.val();
            if (Array.isArray(data)) {
                data.forEach(k => { if(k && typeof k === 'string' && k.trim()) keys.push(k.trim()); });
            }
        }
    ...
    if (keys.length === 0) {
        try {
            ...
            if (storedSettings) {
                ...
                if (parsed.groqApiKeys && Array.isArray(parsed.groqApiKeys)) {
                    parsed.groqApiKeys.forEach(k => { 
    ...
    // 3. HARD FALLBACK
    if (keys.length === 0) {
        console.warn("Using Fallback Groq Key");
    
    }
    
    const valid = Array.from(new Set(keys)).filter(k => k.length > 5); 
    if (valid.length === 0) {
        console.warn("No Groq API Keys found (Restricted Mode)");
    } else {
        console.log(`Loaded ${valid.length} Groq API Keys.`);
    }
    return valid;
};
...
export const executeWithRotation = async <T>(
    ...
    // QUOTA CHECK (Reusing logic from Gemini service)
    try {
    ...
    if (keys.length > 0) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            try {
                ...
                // TRACK USAGE (We track per key index even if keys change, it's rough estimation)
                ...
                if (msg.includes("401") || msg.includes("invalid_api_key")) {
                    console.error(`Invalid Groq API Key found: ...${key.slice(-4)}`);
                } else if (msg.includes("429")) {
                    console.warn(`Groq API Key ...${key.slice(-4)} Rate Limited. Rotating...`);
                } else {
                    console.warn(`Groq API Key ...${key.slice(-4)} failed. Trying next. Error: ${msg}`);
                }
            }
        }
    }
...
const executeBulkParallel = async <T>(
    ...
    console.log(`🚀 Starting Bulk Engine (Groq): ${tasks.length} tasks with ${keys.length} keys (Parallelism: ${concurrency})`);
...
export const translateToHindi = async (content: string, isJson: boolean = false, usageType: 'PILOT' | 'STUDENT' = 'STUDENT'): Promise<string> => {
    ...
    return await executeWithRotation(async (key) => {
        return await callGroqApi(key, [{ role: "user", content: prompt }]);
    }, usageType);
};
...
export const fetchChapters = async (
  ...
  try {
    const data = await executeWithRotation(async (key) => {
        const content = await callGroqApi(key, [
...
export const fetchLessonContent = async (
  ...
  const generateNotes = async (detailed: boolean): Promise<{text: string, hindiText?: string}> => {
      ...
      if (template) {
          ...
          Write ${detailed ? 'PREMIUM DEEP ANALYSIS' : 'SUMMARY'} study notes for ${board} Class ${classLevel} ${subject.name}, Chapter: "${chapter.title}".
          Language: ${language}.
          ${competitionConstraints}
          
          ${!adminPromptOverride ? `STRICT PROFESSIONAL GUIDEBOOK MODE
          
          NEGATIVE CONSTRAINTS:
          - NO Conversational Filler (e.g., "Hello students", "Let's learn").
          - NO Direct Address (e.g., "You").
          
          POSITIVE INSTRUCTIONS:
          - Use Professional Labels: "🔥 MOST IMPORTANT", "🧠 REMEMBER THIS", "⚠️ EXAM ALERT".
          
          STRUCTURAL RULES:
          1. Hook Start: Start with a Thinking Question.
          2. Deep Breakdown: Use Comparison Tables and Flowcharts (Text-based).
          3. Special Sections: 
             - 💡 Concept Chamka? (Insight)
             - ⚠️ Exam Trap (Alert)
             - 🏆 Topper's Trick
          
          Structure:
          1. Introduction (Hook)
          2. Key Concepts (Deep Analysis)
          3. Detailed Explanations (with Tables/Flowcharts)
          4. Important Formulas/Dates
          5. Summary` : ''}

          ${detailed ? 'Include deep insights, memory tips, and exam strategies.' : 'Keep it concise and clear. Focus on quick revision.'}`;
      }

      const text = await executeWithRotation(async (key) => {
          return await callGroqApi(key, [
  ...
  if (dualGeneration && (type === 'NOTES_PREMIUM' || type === 'NOTES_SIMPLE')) {
       ...
       const rawText = await executeWithRotation(async (key) => {
          return await callGroqApi(key, [{ role: "user", content: prompt }], modelName);
       }, usageType);
